    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-end justify-between gap-3">
        <div className="min-w-0">
          ...
          <h1 className="mt-1 truncate text-[21px] font-semibold text-[#241f4d] sm:text-[26px]">Kelola Ujian</h1>
          <p className="mt-1 text-[12px] text-[#8b87a8] sm:text-[13px]">Buat, ubah, dan pantau seluruh ujian di sini.</p>
        </div>
        <Link
          href="/ujian/create"
      ...
      {daftarUjian.length === 0 ? (
        <Card className="px-6 py-16 text-center">
          <IconDocument className="mx-auto h-10 w-10 text-[#c1b8f2]" />
          <p className="mt-3 text-[14px] font-medium text-[#241f4d]">Belum ada ujian</p>
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {daftarUjian.map((ujian) => {
            ...
            return (
              <Card
                key={ujian.id}
                className="group relative flex flex-col overflow-hidden border-[#e6e3f7] p-5 shadow-[0_10px_24px_-18px_rgba(37,26,110,0.4),0_4px_0_#f5f4fb] transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_18px_32px_-18px_rgba(37,26,110,0.45),0_5px_0_#ecebfa]"
              >
                <span className="pointer-events-none absolute right-0 top-0 h-24 w-24 rounded-bl-[60px] bg-gradient-to-bl from-[#ece9fe] to-transparent opacity-80" />
                ...
                <div className="relative flex items-start justify-between gap-3">
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#7c6fce]">Ujian</p>
                      <h2 className="mt-1 truncate text-[16.5px] font-semibold text-[#241f4d]">{ujian.judul}</h2>
                    </div>
                  </div>
                  <span
                ...
                {ujian.deskripsi && (
                  <p className="relative mt-3 line-clamp-2 text-[12.5px] text-[#8b87a8]">{ujian.deskripsi}</p>
                )}

                <p className="relative mt-3 text-[11.5px] text-[#8b87a8]">
                ...
                <div className="relative mt-4 grid grid-cols-3 gap-2.5">
                  <div className="flex flex-col items-center gap-1.5 rounded-[12px] border border-[#ece9fa] bg-[#faf9ff] px-2 py-3">
                    <span className="flex h-9 w-9 items-center justify-center rounded-[10px] bg-gradient-to-br from-white to-[#f1effc] shadow-[inset_0_1px_1px_rgba(255,255,255,0.85),0_4px_10px_-5px_rgba(67,56,202,0.4)] sm:h-11 sm:w-11">
                      <Image src="/image/icon/durasi-icon.png" alt="" width={32} height={32} className="h-6 w-6 object-contain sm:h-7 sm:w-7" />
                    </span>
                    <p className="text-[13px] font-semibold text-[#241f4d]">{ujian.durasiMenit}′</p>
                    <p className="text-[10px] text-[#a29cc4]">Durasi</p>
                  </div>
                  <div className="flex flex-col items-center gap-1.5 rounded-[12px] border border-[#ece9fa] bg-[#faf9ff] px-2 py-3">
                    <span className="flex h-9 w-9 items-center justify-center rounded-[10px] bg-gradient-to-br from-white to-[#f1effc] shadow-[inset_0_1px_1px_rgba(255,255,255,0.85),0_4px_10px_-5px_rgba(67,56,202,0.4)] sm:h-11 sm:w-11">
                      <Image src="/image/icon/soal-icon.png" alt="" width={32} height={32} className="h-6 w-6 object-contain sm:h-7 sm:w-7" />
                    </span>
                    <p className="text-[13px] font-semibold text-[#241f4d]">{ujian._count.soal}</p>
                    <p className="text-[10px] text-[#a29cc4]">Soal</p>
                  </div>
                  <div className="flex flex-col items-center gap-1.5 rounded-[12px] border border-[#ece9fa] bg-[#faf9ff] px-2 py-3">
                    <span className="flex h-9 w-9 items-center justify-center rounded-[10px] bg-gradient-to-br from-white to-[#f1effc] shadow-[inset_0_1px_1px_rgba(255,255,255,0.85),0_4px_10px_-5px_rgba(67,56,202,0.4)] sm:h-11 sm:w-11">
                      <Image src="/image/icon/peserta-icon.png" alt="" width={32} height={32} className="h-6 w-6 object-contain sm:h-7 sm:w-7" />
                    </span>
                    <p className="text-[13px] font-semibold text-[#241f4d]">{ujian._count.hasilUjian}</p>
                    <p className="text-[10px] text-[#a29cc4]">Peserta</p>
                  </div>
                </div>

                <div className="relative mt-5 flex items-center justify-between border-t border-[#f1f0f8] pt-4">
                  <div className="flex flex-wrap items-center gap-1">
                    <Link
                      href={`/ujian/${ujian.id}/soal`}
                      className="inline-flex items-center gap-1 rounded-[8px] px-2 py-1.5 text-[12px] font-medium text-[#4338ca] hover:bg-[#f2f1fc]"
                    >
