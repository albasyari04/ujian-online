...
function IconBell({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} fill="none" aria-hidden="true">
      <path d="M6 10.5C6 7 8.7 4.5 12 4.5C15.3 4.5 18 7 18 10.5V14L19.5 16.5H4.5L6 14V10.5Z" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" />
...
function AvatarGuru({ photoUrl, nama, className = "h-7 w-7" }: { photoUrl?: string | null; nama: string; className?: string }) {
  return (
    <span className={`relative inline-block shrink-0 overflow-hidden rounded-full bg-[#e7e4dc] dark:bg-white/10 ${className}`}>
      <Image src={photoUrl && photoUrl.trim() !== "" ? photoUrl : "/image/icon/user-icon.png"} alt={nama} fill sizes="40px" className="object-cover" />
    </span>
  )
}
...
export function NavbarGuru({
  ...
  function handleSearchSubmit(event: FormEvent) {
    ...
    if (!trimmed) return
    router.push(`/ujian-guru?cari=${encodeURIComponent(trimmed)}`)
  }

  const namaDepan = user.nama.trim().split(" ")[0] || "Guru"
  ...
  return (
    <header className="relative z-20 flex h-[64px] shrink-0 items-center gap-3 border-b border-[#edf0ef] bg-white px-4 dark:border-white/10 dark:bg-[#0b1120] sm:px-6 lg:px-7">
      <button
        type="button"
        onClick={onMenuClick}
        className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] text-[#4338ca] hover:bg-[#eef2ff] dark:text-[#818cf8] dark:hover:bg-white/10 lg:hidden"
      ...
      <form
        ...
        className="hidden max-w-[340px] flex-1 items-center gap-2 rounded-[11px] border border-[#edf0ef] bg-[#f7f9f8] px-3.5 py-2 transition-colors focus-within:border-[#a5b4fc] focus-within:bg-white dark:border-white/10 dark:bg-white/5 dark:focus-within:border-[#818cf8] dark:focus-within:bg-white/10 sm:flex"
      >
      ...
      <div className="flex flex-1 items-center justify-end gap-3">
        <Link
          ...
          className="flex h-9 w-9 items-center justify-center rounded-full text-[#4338ca] hover:bg-[#eef2ff] dark:text-[#818cf8] dark:hover:bg-white/10"
        ...
        <div className="relative" ref={menuRef}>
          <button
            ...
            className="flex items-center gap-2 rounded-full border border-[#e7e4dc] py-1 pl-1 pr-2.5 shadow-[0_2px_6px_rgba(49,46,129,0.05)] hover:bg-[#f6f4ec] dark:border-white/10 dark:hover:bg-white/10"
          >
            <AvatarGuru photoUrl={user.photoUrl} nama={user.nama} className="h-7 w-7 ring-1 ring-[#e7e4dc] dark:ring-white/10" />
          ...
          {menuOpen && (
            <div className="absolute right-0 top-[calc(100%+8px)] z-50 w-[220px] rounded-[14px] border border-[#e7e4dc] bg-white p-2 shadow-[0_16px_36px_rgba(49,46,129,0.16)] dark:border-white/10 dark:bg-[#0f1b30] dark:shadow-[0_16px_36px_rgba(0,0,0,0.4)]">
              <div className="flex items-center gap-2.5 border-b border-[#efece4] px-2.5 pb-2.5 pt-1 dark:border-white/10">
                <AvatarGuru photoUrl={user.photoUrl} nama={user.nama} className="h-9 w-9" />
